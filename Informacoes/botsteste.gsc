init()
{
	...
	// level thread showTeamTimes();
	level thread botLookatThread();
...
showTeamTimes()
{
	for(;;)
	{
		...
		{
			p = level.players[i];
			p iprintln(p.timePlayed["allies"] + " " + p.timePlayed["axis"] + " " + p.timePlayed["other"] + " " + p.timePlayed["total"]);
		}
...
botLookAtMe()
{
	...
	{
		if(!isDefined(players[i].pers["isBot"])) // is not a bot
		{
	...
	//iprintln(firstPlayer.origin);

	for(i=0;i<players.size;i++)
	{
		if(isDefined(players[i].pers["isBot"])) // is a bot
		{
			bot = players[i];
			bot botStop();
			...
			//bot botaction("+fire");
			//iprintln(bot.name + " fire!");
			
			iprintln(bot.name + " look!");
			bot botLookAt(firstPlayer.origin, 0.5);
			//bot botLookAtPlayer(firstPlayer, "head");
			/*bot botmoveto(firstPlayer.origin);
			bot botaction("+fire");*/
		}
	}
