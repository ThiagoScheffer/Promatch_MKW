...
//versao baseada no original OW
//V1 simplificado algumas coisas para funcionar como o update5
//V2 futura. recebera varias "faces" q o jogador podera montar no personagem.
...
init()
{
	level.scr_cap_enable = 0;
	level.scr_cap_time = 5;//getdvarx( "scr_cap_time", "float", 5.0, 1.0, 15.0 );
	level.scr_cap_activated = getdvarx( "scr_cap_time_activated", "float", 15.0, 5.0, 30.0 );
	level.scr_cap_firstspawn = 1;//getdvarx( "scr_cap_firstspawn", "int", 0, 0, 1 );
...
customizePlayer()
{
	...
	passedTime = promatch\_timer::getTimePassed();
	...
	{
		timeDifference = promatch\_timer::getTimePassed() - passedTime;
		wait .01;
	}
	...
	self thread promatch\_speedcontrol::setModifierSpeed( "_capeditor", 100 );
	...
	self thread promatch\_speedcontrol::setModifierSpeed( "_capeditor", 0 );
