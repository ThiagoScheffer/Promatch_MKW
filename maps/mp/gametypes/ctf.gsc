...
main()
{
	...
	// if ( !isdefined( game["switchedsides"] ) )//207 Fix no switch team
	...
	maps\mp\gametypes\_globallogic::registerScoreLimitDvar( level.gameType, 4, 0, 5000 );
...
GiveScorePlayersTeamEVP(team)
{
	for ( index = 0; index < level.players.size; index++ )
	{
		...
		if ( player.pers["team"] == team ){player GiveEVP(100,100);}		
	}
}
...
onPickup( player )
{
	...
	//playerTeam != self.ownerTeam = meu time pegou dos inimigos
	...
	if ( playerTeam != self.ownerTeam && self.curOrigin == self.trigger.baseOrigin ) 
	{
		...
		player GiveEVP(70,90);
	}

	// Play the corresponding sounds for players
	if ( playerTeam != self.ownerTeam ) 
	{
		if(!player.incognito)//eu nao revelo nada ser for inc
		{
			...
			statusDialog( "enemyflag", playerTeam,player);
			statusDialog( "ourflag", getOtherTeam( playerTeam ),player );
		}
	}
	else 
	{
	...
	player thread maps\mp\gametypes\_gameobjects::attachUseModel( game[getdvar("g_gametype")]["prop_flag_" + self.ownerTeam + "_carry" ], "J_SpineLower", true );//verificar 207
...
monitorFlagCarrier( flagCarrier )
{
	...
	//{
		...
		{
			// Wait the time
			xWait( level.scr_ctf_show_flag_carrier_time,false );
...
onDrop( player )
{
	...
	if ( isDefined( player ) ) 
	{
		...
		if ( isAlive( player ) ) 
		{
			player maps\mp\gametypes\_gameobjects::detachUseModels();
	...
	statusDialog( "ourflag_drop", self.ownerTeam,player );
	statusDialog( "enemyflag_drop", getOtherTeam( self.ownerTeam ),player );

	// Make the flag visible to everyone
	self maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", game[getdvar("g_gametype")]["compass_waypoint_flag_" + self.ownerTeam ] );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", game[getdvar("g_gametype")]["waypoint_flag_" + self.ownerTeam ] );
	self maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", game[getdvar("g_gametype")]["compass_waypoint_flag_" + self.ownerTeam ] );
	self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", game[getdvar("g_gametype")]["waypoint_flag_" + self.ownerTeam ] );
	self maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
...
onReset( player )
{
	...
	statusDialog( "ourflag_return", self.ownerTeam,player );
	statusDialog( "enemyflag_return", getOtherTeam( self.ownerTeam ),player );
}
	...
	if ( player.isFlagCarrier ) 
	{
		...
		if ( playerTeam == player.carryObject.ownerTeam || level.flags[playerTeam].curOrigin == level.flags[playerTeam].trigger.baseOrigin ) {
			...
			if ( playerTeam != player.carryObject.ownerTeam ) 
			{
				...
				if(!player.incognito)
				{
					statusDialog( "enemyflag_capt", player.pers["team"],player );
					statusDialog( "ourflag_capt", getOtherTeam( player.pers["team"]),player );
				}
...
resetObjectiveWaypoints( flagAtHome )
{
	...
	} else 
	{
		...
		level.zones[self.ownerTeam] maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", game[getdvar("g_gametype")]["waypoint_flag_" + self.ownerTeam + "_x"] );
		level.zones[self.ownerTeam] maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", game[getdvar("g_gametype")]["waypoint_flag_" + self.ownerTeam + "_x"] );
	}
}



statusDialog( dialog, team, player)
{
	time = getTime();
	if ( time < level.lastStatus[team] + 5000 )
		return;

	thread delayedLeaderDialog( dialog, team );
	
	level.lastStatus[team] = time;
}
