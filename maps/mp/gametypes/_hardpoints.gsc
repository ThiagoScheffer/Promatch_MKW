...
init()
{
	// Get module's dvars
	// General dvars
	level.scr_game_forceuav = getdvarx( "scr_game_forceuav", "int", 1, 0, 1 );
	...
	level.scr_hardpoint_airstrike_streak = getdvarx( "scr_hardpoint_airstrike_streak", "int", 6, 1, 50 );
	...
	level.scr_hardpoint_helicopter_streak = getdvarx( "scr_hardpoint_helicopter_streak", "int", 10, 1, 50 );
	...
	level.radarViewTime = 10; // time radar remains active
...
callStrike( owner, coord, yaw )
{
	...
	//xwait( randomfloatrange( 1.5, 2.5 ), true );
...
triggerHardpoint( hardpointType )
{
	...
	else if ( hardpointType == "airstrike_mp" )
	{
		...
		if ( level.scr_airstrike_hardpoint_interval > 0 ) {
			...
			currentTime = promatch\_timer::getTimePassed() / 1000;
		...
		if ( level.teambased ) {
			team = self.pers["team"];
			if ( team == "allies" )
			level.allies_last_airstrike = promatch\_timer::getTimePassed() / 1000;
			else
			level.axis_last_airstrike = promatch\_timer::getTimePassed() / 1000;
		} else {
			self.pers["last_airstrike"] = promatch\_timer::getTimePassed() / 1000;
		}
	}
	else if ( hardpointType == "helicopter_mp" )
	{
		...
		if ( level.teambased ) 
		{
			team = self.pers["team"];
			if ( team == "allies" )
			level.allies_last_heli = promatch\_timer::getTimePassed() / 1000;
			else
			level.axis_last_heli = promatch\_timer::getTimePassed() / 1000;
		} else {
			self.pers["last_heli"] = promatch\_timer::getTimePassed() / 1000;
		}
		thread maps\mp\_helicopter::heli_think( self, startnode, self.pers["team"] );
	}
	else if ( hardpointType == "gl_mp" )
   {
      ...
      result = self thread maps\mp\_nuke::nuke();
   }
	return true;
}
...
UAVAcquiredPrintAndSound( team, otherteam, callingPlayer, numseconds )
{
	...
	for ( i = 0; i < level.players.size; i++ )
	{
		...
		if ( isdefined( playerteam ) )
		{
			if ( playerteam == team )
			{
				player iprintln( "[RADAR ATIVO] ", callingPlayer, " " + numseconds );
...
useRadarItem()
{
	...
	else
	{
		...
		//self iprintln( "[RADAR ATIVO] ", self, " " + level.radarViewTime );
...
useTeamUAV( team, otherteam )
{
	...
	//finishWait = promatch\_timer::getTimePassed() + level.radarViewTime * 1000;
	
	while ( isdefined(level.spyplane )) 
	{
		wait level.oneFrame;
...
usePlayerUAV( team, otherteam )
{
	...
	finishWait = promatch\_timer::getTimePassed() + level.radarViewTime * 1000;
	while ( isAlive(self) && finishWait > promatch\_timer::getTimePassed() ) {
		wait level.oneFrame;
	}	
	...
	self iprintln( "RADAR FINALIZADO" );
}
