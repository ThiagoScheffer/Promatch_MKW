...
addSpawnPoints( team, spawnPointName )
{
	...
	if ( !level.teamSpawnPoints[team].size )
	{
		...
		xwait( 1.0, false ); // so we don't try to abort more than once before the frame ends
		return;
	}
...
placeSpawnPoints( spawnPointName )
{
	...
	if ( !spawnPoints.size )
	{
		println( "^1No " + spawnPointName + " spawnpoints found in level!" );
		maps\mp\gametypes\_callbacksetup::AbortLevel();
		xwait( 1.0, 0 ); // so we don't try to abort more than once before the frame ends
		return;
	}
