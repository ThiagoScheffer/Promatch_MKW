...
init()
{
	...
	level.destructibleSpawnedEntsLimit = 15;
...
{	
	...
	for(;;)
	{
		...
		//pair 'undefined' and '' has unmatching types 'undefined' and 'string': (file 'maps/mp/_destructible.gsc', line 428)
		if ( !isdefined( modelName ) )
			continue;
		...
		//added to prevent shots to hit and destroy itens on prematch
		if ( level.inStrategyPeriod || level.inPrematchPeriod )
			continue;
...
physics_launch( partIndex, stateIndex, point, initial_velocity )
{
	...
	if ( !level.scr_destructibles_enable_physics )
		return;
